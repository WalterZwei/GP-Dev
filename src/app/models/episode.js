...
Episode.prototype.downloadingCallback = function(event) {
	...
	if (event.returnValue === false &&
		...
		Mojo.Log.warn("Download complete, completionStatusCode=%d!", this.title, event.completionStatusCode );
		this.downloading = false;
		this.downloadingPercent = 0;
		this.downloadActivity();
		...
		if (event.completionStatusCode === 200) {
		...
		} else if (event.completionStatusCode === -5) {  // http-Errpr
			...
			Util.showError($L({value: "Download Failed", key:"downloadFailed"}),
						   $L({value: "Failure downloading #{title}.", key:"downloadFailedDetail"}).interpolate({title:this.title}));
		} else {
			this.downloadTicket = 0;
			Mojo.Log.error("CompletionStatusCode=%d, file=%s, errorCode=%s", event.completionStatusCode, event.destPath + event.destFile, event.errorCode);
		}
