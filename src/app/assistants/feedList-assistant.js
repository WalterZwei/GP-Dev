...
FeedListAssistant.prototype.addMenuModel = {
	items: [{label: $L({value:"Enter feed URL...", key:"enterFeedURL"}), command: "add-feed"},
	        ...
	        //{label: "TestMediapermisson...", command: "test-perm"}
	        ]
};
	...
	if (Prefs.reload) {
		...
		if (Prefs.firstRun) {
			...
			var dialog = new drnull.Dialog.Confirm(this, $L({value:"Add Default Feeds", key:"addDefaultFeeds"}),
				$L({value:"Welcome to GuttenPodder!<br><br>Would you like to add some podcasts to get you started?", key:"drpodderWelcome"}),
				function() {
					var dialog = new drnull.Dialog.Info(this, $L({value:"Thanks for using GuttenPodder!", key:"drpodderThanks"}),
...
FeedListAssistant.prototype.handleCommand = function(event) {
    if (event.type == Mojo.Event.command) {
        switch (event.command) {
			...
			case "test-perm":
               var outthis = this;
               this.checkPermissions(function continueToRunApp(e) {  
                    //we're running good now!  
                   Mojo.Log.warn("Permisson ok");

                   var results = e.results;
                   var s = "";
                   for (i=0; results[i] != null; i++) {
                      if( results[i].isRingtone ) {
                         // Mojo.Log.info("ringtone #"+i+"  path ="+ results[i].path);
                      } else {
                         s = s  + results[i].path + '<br>';
                      }
                   }
                   outthis.controller.get("mediatest").update(  s  );
                }); 
                break;
 /*               
[{"_id":"++I5ucZbGb0rEhF I",
...
"_rev":551,
"album":"webOS Alert Sounds" ,
"albumArtist":"Palm, Inc.",
"artist":"Palm, Inc.",
"bookmark":0,
"createdTime":0, 
" disc":{"position":1, "total":1},
"duration":0,
"genre":"Alert Sounds",
"hasResizedThumbnails":false,
"isRingtone":true,
"modifiedTime":1344617672,
"path":"/media/internal/ringtones/Triangle (short).mp3",
"searchKey":"Palm, Inc.\t\twebOS Alert Sound s\t\tTriangle (short)",
"serviced":false,
"size":37788,
"sortKey":{"albumArtistDisc AndTrack":"webOS Alert Sounds\t\tPalm, Inc.\t\t100000",
    "albumDiscAndTrack":"webOS Alert Sounds\t\t100000",
     "artistAlbumDiscAndTrack":"Palm, Inc.\t\twebOS Alert S ounds\t\t100000",
     "trackAndDisc":"100000"},
"thumbnails":[{"_id":"228",
                   "data":"/media/internal/ringtones/Triangle (short).mp3:234:3015",
        "type":"embedded"}],
"title":"Triangle (short)",
"track":{"position":0, "total":0}}]
...
FeedListAssistant.prototype.checkPermissions = function(done){  
    self = this;
    this.controller.serviceRequest('palm://com.palm.db', {  
        ...
        onFailure: function(e) {  
            Mojo.Log.info("find failure! Err = " + JSON.stringify(e));
            Mojo.Log.info('We do not have permission, asking.');  
            self.requestPermission(done(e));  
        }  
    });  
}


FeedListAssistant.prototype.requestPermission = function(done) { 
